...
(defun org-jira--render-dev-status-branches (branches)
  (let ((grouped (-group-by (lambda (branch) (org-jira-sdk-path branch '(repository url))) branches))
        (i 0))
    (mapc (lambda (group)
            (let* ((branches (cdr group))
                   (branch (car branches))
                   (repo-url (org-jira-sdk-path branch '(repository url)))
                   (repo-name (org-jira-sdk-path branch '(repository name)))
                   (j 0))
              (if (eq i 0)
                  (org-insert-subheading t)
                (progn
                  (org-insert-heading)
                  (org-promote)))
              (org-jira-insert (format "%s ([[%s][Link]])" repo-name repo-url) "\n")
              (setq i (+ i 1))
              (mapc (lambda (branch)
                      (let* ((branch-name (org-jira-sdk-path branch '(name)))
                             (branch-url (org-jira-sdk-path branch '(url)))
                             (commit-id (org-jira-sdk-path branch '(lastCommit displayId)))
                             (commit-url (org-jira-sdk-path branch '(lastCommit url)))
                             (commit-date (org-jira-sdk-path branch '(lastCommit authorTimestamp)))
                             (commit-date-str (string-replace "T" " " commit-date)))
                        (if (eq j 0)
                            (org-insert-subheading t)
                          (org-insert-heading))
                        (org-jira-insert (format "%s ([[%s][Link]])" branch-name branch-url) "\n")
                        (setq j (+ j 1))
                        (org-jira-entry-put (point) "latest-commit" (format "[[%s][%s]]" commit-url commit-id))
                        (org-jira-entry-put (point) "updated" commit-date-str)))
                    branches)))
          grouped)
    ))

(defun org-jira--render-dev-status-pull-requests (pull-requests)
  (let ((grouped (-group-by (lambda (pr) (org-jira-sdk-path pr '(repositoryName))) pull-requests))
                (i 0))
    (mapc (lambda (group)
            (let* ((prs (cdr group))
                   (pr (car prs))
                   (repo-url (org-jira-sdk-path pr '(repositoryUrl)))
                   (repo-name (org-jira-sdk-path pr '(repositoryName)))
                   (j 0))
              (if (eq i 0)
                  (org-insert-subheading t)
                (progn
              ...
              (setq i (+ i 1))
              (mapc (lambda (pr)
                      (let* ((pr-id (org-jira-sdk-path pr '(id)))
                             (pr-name (org-jira-sdk-path pr '(name)))
                             (pr-url (org-jira-sdk-path pr '(url)))
                             (pr-author-name (org-trim (org-jira-sdk-path pr '(author name))))
                             (pr-src-url (org-jira-sdk-path pr '(source url)))
                             (pr-src-branch (org-jira-sdk-path pr '(source branch)))
                             (pr-dest-url (org-jira-sdk-path pr '(destination url)))
                             (pr-dest-branch (org-jira-sdk-path pr '(destination branch)))
                             (pr-status (org-jira-sdk-path pr '(status)))
                             (pr-updated (org-jira-sdk-path pr '(lastUpdate))))
                        (if (eq j 0)
                            (org-insert-subheading t)
                          (org-insert-heading))
                        (org-jira-insert (format "%s [[%s][%s]] - %s" pr-status pr-url pr-id pr-name) "\n")
                        (setq j (+ j 1))
                        (org-jira-entry-put (point) "author" pr-author-name)
                        (org-jira-entry-put (point) "source" (format "[[%s][%s]]" pr-src-url pr-src-branch))
                        (org-jira-entry-put (point) "destination" (format "[[%s][%s]]" pr-dest-url pr-dest-branch))
                        (org-jira-entry-put (point) "updated" pr-updated)))
                    prs)))
          grouped)))
...
(defun org-jira--render-dev-status (dev-status)
  ...
  (let* ((detail (car (append (org-jira-sdk-path dev-status '(detail)) nil)))
         (branches (append (org-jira-sdk-path detail '(branches)) nil))
         (pull-requests (append (org-jira-sdk-path detail '(pullRequests)) nil)))
    (org-insert-subheading t)
    (org-jira-insert "Branches" "\n")
    (org-jira--render-dev-status-branches branches)
    (org-insert-heading)
    (when (> (length branches) 0)
      (org-promote))
    (org-jira-insert "Pull Requests" "\n")
    (org-jira--render-dev-status-pull-requests pull-requests)))
